...
cc.Class({
    ...
    onLoad() {
        ...
        // 初始化数据
        this.level = 2; // 柔化等级，即网格的层数
        this.soft = 5;// 拖尾的柔软度，值越大尾巴越柔软，拉伸的越长
        this.startPosition = cc.v3(0, 0, 0); // 起始坐标
        this._playing = false; // 是否在播放的标志
        this.followAngle = false; // 跟随目标角度
        // this.offset = cc.v3(0, 0, 0); // 拖尾中心点相对目标节点的坐标的偏移量【暂时不需要偏移】
        // 初始化网格
    ...
    setShape() {
        this.polygon = [cc.v2(0, 2), cc.v2(2, 0), cc.v2(0, -2), cc.v2(-2, 0)];
        // cc.log("顶点数组长度：" + this.polygon.length + "," + this.polygon[2].toString());
        this.createVerts(); // 根据截面形状和拖尾位置创建网格顶点111
    ...
    // start() {
    //      this.play(cc.v3(this.target.x, this.target.y, this.target.children[0].z)); // 开始播放
    // },

    // 自动停止
    autoStop() {
        this.oldPos = cc.v3(this.target.x, this.target.y, this.targetChild.z); // 保存目标最后一帧的坐标
        this._playing = false;
        // 一定时间后传入对象池
    ...
    returnPool() { // 将本节点返回对象池
        // 回收对象
        this.bCtrl.trailReturn(this.node);
    },

    // 外部调用的开始播放，传入目标
    autoPlay(target) {
        this.target = target; // 设置目标
        this.targetChild = target.children[0]; // 记录子节点(高度属性)
        let pos = cc.v3(this.target.x, this.target.y, this.targetChild.z); // 目标坐标
    ...
    update(dt) {
        if (!this._playing || !this.target) // 不播放或无目标
        {
            let pos = this.oldPos;
            // pos.addSelf(this.offset); // 加上偏移量
            if (!this.followAngle) {
